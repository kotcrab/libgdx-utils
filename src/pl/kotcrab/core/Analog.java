...
public class Analog implements Disposable
{
	...
	private static final boolean[] pointerTab = new boolean[5]; // tablica uzywanych paclow wspolna dla wszyskich klas(moze obsluzyc 5 analogw naraz(latwo ziwkeszyc))
	...
	public void update()
	{
		if(updating == false) // jezeli anlog nie jest updatedowany
		{
			for (int i = 0; i < pointerTab.length; i++)
			{
				...
				if(Gdx.input.isTouched(i) == true) // palac nie w tablicy a ekran dotkniety
				{
					...
					if(base.getBoundingRectangle().contains(Touch.calcX(Gdx.input.getX(activePointer)), Touch.calcY(Gdx.input.getY(activePointer))))
					{
		...
		float x = Touch.calcX(Gdx.input.getX(activePointer)); // pobranie prawdidlowych danych
		float y = Touch.calcY(Gdx.input.getY(activePointer));
	...
	/**
	 * Odblokowyuje uzywamy palec. Musi zostac wywoalne po skonczoeniu uzywania klasy, inaczej aktualny palec caly czas bedzie zablokowany
	 */
	@Override
	public void dispose()
	{
